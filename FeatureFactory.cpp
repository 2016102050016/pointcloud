...
FeatureFactory::FeatureFactory(Eigen::MatrixXf& neighborhood, int featType, int point1, int point2) :
	_featType(featType),
...
{
	return _neighborhood(_point1, 4) < _neighborhood(_point2, 4) ? true : false;
}

inline bool FeatureFactory::greenColorDiff()
{
	return _neighborhood(_point1, 5) < _neighborhood(_point2, 5) ? true : false;
}

inline bool FeatureFactory::blueColorDiff()
{
	return _neighborhood(_point1, 6) < _neighborhood(_point2, 6) ? true : false;
}

inline bool FeatureFactory::xDiff()
{
	return _neighborhood(_point1, 0) < _neighborhood(_point2, 0) ? true : false;
}

inline bool FeatureFactory::yDiff()
{
	return _neighborhood(_point1, 1) < _neighborhood(_point2, 1) ? true : false;
}

inline bool FeatureFactory::zDiff()
{
	return _neighborhood(_point1, 2) < _neighborhood(_point2, 2) ? true : false;
}
